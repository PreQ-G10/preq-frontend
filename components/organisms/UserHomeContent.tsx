...
import { styles } from './UserHomeContent.styles';
...
export function UserHomeContent() {
  ...
  useEffect(() => {
    ...
    if (trimmed.length < 3) { setShowResults(false); setResults([]); return; }
      ...
      >
        <View style={styles.searchWrapper}>
          <SearchBar
            ...
            onSubmit={() => { setShowResults(false); router.push({ pathname: Routes.search, params: { query } }); }}
            ...
            <View style={styles.dropdown}>
              ...
              {!searching && results.slice(0, 3).map(data => (
                <TouchableOpacity
                  ...
                  onPress={() => { setShowResults(false); router.push(Routes.productDetails(data.product.id)); }}
                  activeOpacity={0.7}
                >
              ...
              {!searching && results.length > 3 && (
                <TouchableOpacity
                  ...
                  onPress={() => { setShowResults(false); router.push({ pathname: Routes.search, params: { query } }); }}
                >
        ...
        {pendingCount > 0 && (
          <ValidationBanner count={pendingCount} onPress={() => router.push(Routes.validation)} />
        )}
